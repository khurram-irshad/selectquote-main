...
import { formStepSchema10 } from "@common/schema/schema";
...
type FormData = {
    life_sports_high_risk: string;
};

export default function StepTen({ step, quoteForm }: { step: TypeComponent_Quote_Form_Step, quoteForm: TypeComponent_Quote_Form }) {
    ...
    const { register, setValue, handleSubmit, watch, getValues, reset, trigger, formState: { errors, isValid } } = useForm<FormData>({
        resolver: yupResolver(formStepSchema10)
    });

    useEffect(() => {
        let storageData = StorageService.getItem('quote_form');
        if (storageData && storageData.life_sports_high_risk) {
            setIsEditMode(true)//3
            reset({
                life_sports_high_risk: storageData?.life_sports_high_risk,
            })
        }
    ...
    const onSubmit = (data: any) => {
        ...
        let model={ life_sports_high_risk:data.life_sports_high_risk}
        ...
        router.push('/quote-form/step-11');
    };
                ...
                <div className="form-wrapper">
                    ...
                    <section className="forms">
                        <div className="hobbies">
                            <RichTextRenderer text={getQuestionCopy('HOBBIES')} />
                        </div>
                            ...
                            <ul className="form-radio-selectors">
                                <li>
                                    <input
                                        ...
                                        id="Yes"
                                        value="1"
                                        {...register("life_sports_high_risk")}
                                    />
                                ...
                                <div className="text-center">
                                    {errors.life_sports_high_risk && <p role="alert" className="error-text">{errors.life_sports_high_risk?.message}</p>}
                                </div>
                            </ul>
                            <div className="d-flex  justify-content-center mt-5">
                                <div className="d-inline-flex flex-md-row flex-column">
                                    <a href="/quote-form/step-9" className="btn btn-transparent-md order-2 order-md-1">  <RichTextRenderer text={getActionCopy('BACK')} /></a>
